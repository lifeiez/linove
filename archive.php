...
<?php if (is_search()) : ?>
	<div class="boxcaption"><h3><?php _e('Search Results', 'linove'); ?></h3></div>
	<div class="box"><?php printf( __('Keyword: &#8216;%1$s&#8217;', 'linove'), wp_specialchars($s, 1) ); ?></div>

<?php else : ?>
	<div class="boxcaption"><h3><?php _e('Archive', 'linove'); ?></h3></div>
	<div class="box">
		<?php
		// If this is a category archive
		if (is_category()) {
			printf( __('Archive for the &#8216;%1$s&#8217; Category', 'linove'), single_cat_title('', false) );
		// If this is a tag archive
		} elseif (is_tag()) {
			printf( __('Posts Tagged &#8216;%1$s&#8217;', 'linove'), single_tag_title('', false) );
		// If this is a daily archive
		} elseif (is_day()) {
			printf( __('Archive for %1$s', 'linove'), get_the_time(__('F jS, Y', 'linove')) );
		// If this is a monthly archive
		} elseif (is_month()) {
			printf( __('Archive for %1$s', 'linove'), get_the_time(__('F, Y', 'linove')) );
		// If this is a yearly archive
		} elseif (is_year()) {
			printf( __('Archive for %1$s', 'linove'), get_the_time(__('Y', 'linove')) );
		// If this is an author archive
		} elseif (is_author()) {
			_e('Author Archive', 'linove');
		// If this is a paged archive
		} elseif (isset($_GET['paged']) && !empty($_GET['paged'])) {
			_e('Blog Archives', 'linove');
		}
		?>
	</div>
<?php endif; ?>
